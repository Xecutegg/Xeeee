...
export default {
    name: "unlock",
    description: "Unlocks (unhides) a specified channel or the current channel for a specified role or user, or for everyone if no role or user is provided.",
    ...
    async execute(client, message, args) {
        try {
            ...
            if (roleOrUser) {
                ...
                return message.reply(`Successfully unlocked ${channel.name} for ${roleOrUser.name || roleOrUser.user.tag}.`);
            } else {
                await channel.permissionOverwrites.edit(message.guild.id, { ViewChannel: true });
                return message.reply(`Successfully unlocked ${channel.name} for everyone.`);
            }
        } catch (error) {
            console.error(`Error executing unlock command: ${error.message}`);
