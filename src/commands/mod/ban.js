...
export default {
    name: 'ban',
    description: 'Bans a member from the server and notifies them via DM.',
    usage: '<@user> [reason]',
    category: 'moderation',
    ...
    async execute(client, message, args) {
        ...
        if (!member) {
            try {
                ...
                await user.send(`You have been banned from **${guildName}** for the following reason: ${reason}`).catch(() => console.log(`Could not send DM to ${user.tag}.`));
        ...
        try {
            await member.send(`You have been banned from **${guildName}** for the following reason: ${reason}`).catch(() => console.log(`Could not send DM to ${member.user.tag}.`));
