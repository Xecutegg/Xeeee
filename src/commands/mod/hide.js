...
export default {
    name: "hide",
    description: "Hides (locks) a specified channel or the current channel for a specified role or user, or for everyone if no role or user is provided.",
    ...
    async execute(client, message, args) {
        try {
            ...
            if (roleOrUser) {
                ...
                return message.reply(`Successfully hide ${channel.name} from ${roleOrUser.name || roleOrUser.user.tag}.`);
            } else {
                await channel.permissionOverwrites.edit(message.guild.id, { ViewChannel: false });
                return message.reply(`Successfully hide ${channel.name} from everyone.`);
            }
        } catch (error) {
            console.error(`Error executing hide command: ${error.message}`);
