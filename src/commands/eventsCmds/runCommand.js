...
export default {
    ...
    async execute(client, message) {
        ...
        if (message.content === `<@${client.user.id}>`) {
            ...
            let b2 = new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel(`Support`).setURL(`https://discord.gg/WVThZG7Q5X`);
            let b3 = new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel(`Vote`).setURL(`https://discord.gg/WVThZG7Q5X`);
            let ro = new ActionRowBuilder().addComponents(b1, b2, b3);

            let embed = new EmbedBuilder()
                .setColor(Colors.Blue)
                .setFooter({ text: `Made By Xecute ❤️`, iconURL: message.author.displayAvatarURL({ dynamic: true }) })
                .setDescription(
                ...
                .addFields({ name: `__Links__`, value: `[Support](https://discord.gg/WVThZG7Q5X) | [Invite](https://discord.gg/WVThZG7Q5X)` })
                .setAuthor({ name: `Hey I am ${client.user.username}`, iconURL: client.user.displayAvatarURL({ dynamic: true }) })
                .setThumbnail(message.author.displayAvatarURL({ dynamic: true }));
            
            return message.channel.send({ embeds: [embed], components: [ro] }).catch((e) => { 
                message.author.send({ content: `Error while sending message there : ${e.message}` }).catch(() => { }) 
            });
        }
        ...
        const isNonprefixUser = await botconfig.np_users.includes(message.author.id);
        ...
        let command = client.commands.get(cmd) || client.commands.find(cmdObj => cmdObj.aliases && cmdObj.aliases.includes(cmd));

        if (!command) return;
        let devIds = ["162213250719547392", "769225935153004636"];
        ...
        try {
            const loadingEmbed = new EmbedBuilder()
                ...
                .setFooter({ text: "Please wait...", iconURL: client.user.displayAvatarURL() });

            let loadingMessage = await message.reply({ embeds: [loadingEmbed] });
            ...
            const loadingInterval = setInterval(() => {
                ...
                loadingEmbed.setDescription(`⌛ **Processing your request...**\n\`${animationFrames[frameIndex]}\``);
                loadingMessage.edit({ embeds: [loadingEmbed] });
            }, 500);
